...
<template>
  <div class="flex items-center justify-center min-h-screen">
    <div class="w-full max-w-sm p-8 bg-bg-secondary rounded-lg border border-border">
      <h1 class="text-2xl font-semibold text-center mb-8">Create Account</h1>
      ...
      <form @submit.prevent="onSubmit" class="space-y-4">
        ...
        <p v-if="localError || error" class="text-sm text-error">{{ localError || error }}</p>
      ...
      <p class="mt-6 text-center text-sm text-text-secondary">
        ...
        <router-link to="/login" class="text-accent hover:text-accent-hover">Login</router-link>
      </p>
    </div>
  </div>
</template>
